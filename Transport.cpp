...
RESULT Transport_SendPacket(const COMMAND command, const RESULT packet_result, const uint8_t data_length, const int8_t* packet_data) {
  uint8_t sum = 0;
  RESULT result;
  if ((result = SerialDevice_putc(PACKET_STARTING_CHARACTOR_0)) != RESULT::OK) return result;
  if ((result = SerialDevice_putc(PACKET_STARTING_CHARACTOR_1)) != RESULT::OK) return result;
  
  if ((result = SerialDevice_putc((uint8_t)command)) != RESULT::OK) return result;
  sum += (uint8_t)command;
  
  if ((result = SerialDevice_putc((uint8_t)packet_result)) != RESULT::OK) return result;
  sum += (uint8_t)packet_result;

  if ((result = SerialDevice_putc(data_length)) != RESULT::OK) return result;
  sum += data_length;

  for(uint8_t i = 0;i < data_length;i++) {
    sum += packet_data[i];
    if ((result = SerialDevice_putc(packet_data[i])) != RESULT::OK) return result;
  }
  if ((result = SerialDevice_putc(sum)) != RESULT::OK) return result;
  return RESULT::OK;
}
...
RESULT Transport_ReceivePacket(uint8_t* packet, uint8_t* size_read, const duration_t& timeout) {
  ...
  if((ret=SerialDevice_read(packet, PACKET_HEADER_SIZE, &size_read_local, timeout)) != RESULT::OK) return ret;

  // Read multiple bytes according to the header.
  if((ret=SerialDevice_read(packet+PACKET_HEADER_SIZE, packet[DATA_LENGTH], &size_read_local, timeout)) != RESULT::OK) return ret;
  
  // Read last byte. It means checksum.
  if((ret=SerialDevice_read(&buf, 1, &size_read_local, timeout)) != RESULT::OK) return ret;
